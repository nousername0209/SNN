    ...
    def __init__(
        ...
        with FFI_CACHE_LOCK:
            ...
            if key not in FFI_CACHE:
                ...
                print(_load_libenv_cdef())
                if modification:
                    if platform.system() == "Linux":
                        mod_filename = f"lib{modification}.so"
                    elif platform.system() == "Darwin":
                        mod_filename = f"lib{modification}.dylib"
                    elif platform.system() == "Windows":
                        mod_filename = f"{modification}.dll"
                    else:
                        raise Exception(f"unrecognized platform {platform.system()}")
                    ffi.cdef(mod_filename)
    ...
    def _get_space(self, c_name: Any) -> Tuple[typedefs.DictType, List[Spec]]:
        ...
        for i in range(count):
            ...
            if c_tt.scalar_type == self._c_lib.LIBENV_SCALAR_TYPE_REAL:
                ...
                tensortype = typedefs.TensorType(eltype=typedefs.Real(), shape=shape)
                ...
                tensortype = typedefs.TensorType(
                    eltype=typedefs.Discrete(n=high + 1, dtype_name=dtype.name),
                    shape=shape,
                )
            else:
        ...
        return typedefs.DictType(**name_to_tensortype), specs
